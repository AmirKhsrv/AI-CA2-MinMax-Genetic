...
class Board:
    ...
    def getValidMoves(self, piece):
        moves = {}
        whiteStop = min(piece.row + 3, ROWS)
        redStop = max(piece.row - 3, -1)
        if  piece.king or piece.color == RED:
            moves.update(self._traverseLeft(piece.row - 1, redStop, - 1, piece.color, piece.col - 1))
            moves.update(self._traverseRight(piece.row - 1, redStop, - 1, piece.color, piece.col + 1))
        if piece.king or piece.color == WHITE:
            moves.update(self._traverseLeft(piece.row + 1, whiteStop, 1, piece.color, piece.col - 1))
            moves.update(self._traverseRight(piece.row + 1, whiteStop, 1, piece.color, piece.col + 1))
        return moves
